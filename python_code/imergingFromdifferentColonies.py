...
COLLISION_RADIUS = CELL_WIDTH  # Collision radius for agents
...
def draw_grid(screen, grid, robots, charging_stations):
    screen.fill(BLACK)
    ...
    for station in charging_stations:
        pygame.draw.rect(screen, CYAN, (station[1] * CELL_WIDTH, station[0] * CELL_HEIGHT, CELL_WIDTH, CELL_HEIGHT))
...
def main():
    ...
    charging_stations = [(5, 5), (ROWS // 2, COLS // 2), (ROWS - 6, COLS - 6)]  # Three charging stations

    # Robot initialization
    robot_colors = [BLUE, YELLOW, MAGENTA, RED]
    robots = [
        {"position": list(charging_stations[i % len(charging_stations)]), "color": robot_colors[i], "battery": 100,
         "state": "working", "target": None, "charging_station": charging_stations[i % len(charging_stations)]}
        for i in range(4)
    ]
    ...
    while running:
        ...
        for robot in robots:
            if robot["state"] == "working":
                ...
                path = a_star(tuple(robot["position"]), robot["charging_station"], grid)
                if path:
                    robot["position"] = list(path[1]) if len(path) > 1 else list(path[0])
                    robot["battery"] -= 1
                    if tuple(robot["position"]) == robot["charging_station"]:
        ...
        draw_grid(screen, grid, robots, charging_stations)
        clock.tick(10)
